...
//
//
//        when(pedidoPersist.getPedidoById(String.valueOf(pedidoId))).thenReturn(ObjectBuilder.buildProcessarPedidoRequestSucesso());
//
//        mockMvc.perform(post("/pedidos/" + pedidoId + "/processar")
//                        .contentType(MediaType.APPLICATION_JSON))
//                .andExpect(status().isOk());
//
//        verify(pedidoPersist, times(1)).getPedidoById(String.valueOf(pedidoId));
...
//        UUID pedidoId = UUID.randomUUID();
//        when(pedidoPersist.getPedidoById(String.valueOf(pedidoId))).thenThrow(new ResponseStatusException(HttpStatus.NOT_FOUND));
//
//        mockMvc.perform(get("/pedidos/" + pedidoId)
//                        .contentType(MediaType.APPLICATION_JSON))
