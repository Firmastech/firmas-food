...
public class AtualizarPratoUseCaseImpl implements AtualizarPratoUseCase {
    ...
    public AtualizarPratoResponse atualizarPratos(String id, AtualizarPratoRequest pratoRequest) {
        Integer pratoAtualizado = pratoPersist.updatePrato(id, pratoRequest);
        if (pratoAtualizado >= 1) {
            PratoResponse pratoResponse = buscarPratosUseCase.buscarPratoPorId(id);
